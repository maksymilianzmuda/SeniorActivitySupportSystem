...
namespace SeniorActivitySupportSystem.Repository
{
    public class DashboardRepository : IDashboardRepository
    {
        ...
        public async Task<List<SportEvent>> GetAllEvents()
        {
            ...
            var userEvents =  _context.SportEvents.Where(e => e.AppUserId == curUser);
            return userEvents.ToList();
        }

        public async Task<List<SportGroup>> GetAllGroups()
        {
            var curUser = _httpContextAccessor.HttpContext?.User.GetUserId();
            var userGroups = _context.SportGroups.Where(e => e.AppUserId == curUser);
